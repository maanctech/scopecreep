...
- Supported target: self-hosted Next.js, private PostgreSQL, private persistent documents, and Ollama on the host or protected LAN.
  - **Superseded.** The Docker install path is retired to `legacy/docker` because it targets operators rather than the professionals ScopeLedger is sold to. ScopeLedger is now hosted: the customer supplies an account, not infrastructure.
  - **Target: Vercel serverless with managed PostgreSQL,** chosen because server-based hosting is not fundable yet. Three consequences follow and are tracked in [Known Limitations](KNOWN_LIMITATIONS.md). Backups spawn `pg_dump`, `pg_restore` and `tar`, none of which exist on serverless, so installation backup is replaced by managed point-in-time recovery plus tenant-scoped export. Documents move from a local directory to object storage. The startup `config:check` and `rls:check` gates lose their boot step and must run in CI instead.
  - **Hosting makes tenant separation a breach boundary rather than a convenience.** The application must keep connecting as a role holding neither SUPERUSER nor BYPASSRLS, which `scripts/ensure-app-role.ts` provisions and `npm run rls:check` verifies. A managed database hands out a superuser by default, so this matters more under hosting, not less.
