  ...
  return (
    <div className="space-y-20">
      <section className="border-b border-audit-border pb-10">
        <div className="
          flex flex-wrap items-center gap-3 text-sm text-audit-muted
        ">
          <span className="
        ...
        <p className="mt-5 max-w-3xl text-lg/8 text-zinc-700">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <Link href="/request-audit" className="
            inline-flex h-11 items-center gap-2 rounded-md bg-ink px-5 text-sm
        ...
        <div className="
          mt-7 flex flex-wrap gap-x-7 gap-y-2 text-sm text-zinc-700
        ">
          <span className="inline-flex items-center gap-2"><HardDrive className="
            size-4
          " aria-hidden="true" /> Local-model option</span>
          <span className="inline-flex items-center gap-2"><ShieldCheck className="
            size-4
          " aria-hidden="true" /> Human approval required</span>
          <span className="inline-flex items-center gap-2"><XCircle className="
            size-4
          " aria-hidden="true" /> No automated billing</span>
        </div>
        <figure className="
          mt-9 overflow-hidden rounded-md border border-audit-border
          bg-audit-soft shadow-audit
        ">
          <Image
          ...
          <figcaption className="
            border-t border-audit-border px-4 py-3 text-xs text-audit-muted
          ">
      ...
      ">
        ...
        ">
          ...
          ].map((item) => <p key={item} className="
            border-l-2 border-ink pl-4 text-sm/6 text-zinc-700
          ">{item}</p>)}
        </div>
      </section>

      <section id="workflow" className="
        scroll-mt-28 border-y border-audit-border py-12
      ">
        ...
        ">
          {[
            ...
            ">
              ...
              <p className="mt-2 text-sm/6 text-zinc-700">{copy}</p>
            </li>
          ))}
        </ol>
      </section>
      ...
      <section className="
        grid gap-8
        lg:grid-cols-[0.85fr_1.15fr] lg:items-start
      ">
        <div>
          <div className="flex items-center gap-2"><FileSearch className="
            size-5
          " aria-hidden="true" /><h2 className="text-3xl font-semibold">Example detection</h2></div>
          <p className="mt-4 text-base/7 text-zinc-700">
            A request can sound small while introducing design, logic, validation, QA, and another review cycle.
          </p>
        </div>
        <dl className="
          divide-y divide-audit-border border-y border-audit-border text-sm
        ">
          <div className="
            grid gap-2 py-4
            sm:grid-cols-[150px_1fr]
          "><dt className="font-semibold">Client request</dt><dd className="
            text-zinc-700
          ">“This may be quick: can we add an interactive ROI calculator to the Pricing page?”</dd></div>
          <div className="
            grid gap-2 py-4
            sm:grid-cols-[150px_1fr]
          "><dt className="font-semibold">SOW evidence</dt><dd className="
            text-zinc-700
          ">Interactive calculators and pricing estimators are explicitly excluded.</dd></div>
          <div className="
            grid gap-2 py-4
            sm:grid-cols-[150px_1fr]
          "><dt className="font-semibold">Audit estimate</dt><dd className="
            text-zinc-700
          ">Out of Scope, 28 hours at $175/hour, or $4,900 potential leakage.</dd></div>
          <div className="
            grid gap-2 py-4
            sm:grid-cols-[150px_1fr]
          "><dt className="font-semibold">Next action</dt><dd className="
            text-zinc-700
          ">Professional review and a clearly labeled draft change order. Nothing is sent automatically.</dd></div>
        </dl>
      </section>
      ...
      <section id="connections" className="scroll-mt-28">
        ...
        <p className="mt-3 max-w-3xl text-sm/6 text-zinc-700">
        ...
        <div className="
          mt-6 overflow-x-auto rounded-md border border-audit-border
        ">
          <table className="min-w-full text-left text-sm">
            <thead className="bg-audit-soft"><tr><th className="p-3">Source</th><th className="
              p-3
            ">Status</th><th className="p-3">What that means</th></tr></thead>
            <tbody className="divide-y divide-audit-border">
              {connections.map(([source, status, detail]) => <tr key={source}><td className="
                p-3 font-semibold
              ">{source}</td><td className="p-3 whitespace-nowrap">{status}</td><td className="
                p-3 text-zinc-700
              ">{detail}</td></tr>)}
            </tbody>
          </table>
      ...
      ">
        <div>
          <div className="flex items-center gap-2"><LockKeyhole className="
            size-5
        ...
        <div>
          ...
          <ul className="
            mt-4 grid gap-3 text-sm text-zinc-700
            sm:grid-cols-2
          ">
            {["create client accounts", "send client notifications", "approve its own findings", "change an invoice", "charge a payment method", "pull an AI model automatically"].map((item) => <li key={item} className="
              flex gap-2
            "><XCircle className="mt-0.5 size-4 shrink-0 text-zinc-500" aria-hidden="true" />{item}</li>)}
          </ul>
        </div>
      </section>

      <section id="calculator" className="scroll-mt-28">
        <div className="mb-6 flex items-center gap-2"><Calculator className="
          size-5
        " aria-hidden="true" /><h2 className="text-3xl font-semibold">Estimate the leakage</h2></div>
        <RoiCalculator />
      </section>
      ...
      <section>
        ...
        <div className="
          ...
          lg:grid-cols-4
        ">
          {useCases.map(([title, copy]) => <article key={title} className="
            rounded-md border border-audit-border bg-white p-5 shadow-audit
          "><h3 className="font-semibold">{title}</h3><p className="
            mt-3 text-sm/6 text-zinc-700
          ">{copy}</p></article>)}
        </div>
      </section>
      ...
      ">
        <div className="flex items-center gap-2"><Scale className="size-5" aria-hidden="true" /><h2 className="
          text-3xl font-semibold
        ">FAQ</h2></div>
        <div className="space-y-3">
          {faqs.map(([question, answer]) => <details key={question} className="
            rounded-md border border-audit-border bg-white p-4 shadow-audit
          "><summary className="cursor-pointer font-semibold">{question}</summary><p className="
            mt-3 text-sm/6 text-zinc-700
          ">{answer}</p></details>)}
        </div>
      </section>
      ...
      ">
        ...
        ">
          <div><p className="text-sm text-zinc-300">Free manual lookback audit</p><h2 className="
            mt-2 text-3xl font-semibold
          ">Find the revenue already hiding in client requests.</h2></div>
          <Link href="/request-audit" className="
            inline-flex h-11 shrink-0 items-center justify-center rounded-md
            bg-white px-5 text-sm font-semibold text-ink
            hover:bg-zinc-100
          ">Apply for the private beta</Link>
        </div>
      </section>
    </div>
  );
