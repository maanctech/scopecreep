...
export async function createSowVersion(input: { projectId: string; text: string; changeNote?: string | null; extracted?: ExtractedSow; fileBuffer?: Buffer }) {
  ...
  try {
    return await transaction(async (client) => {
      ...
      let document = await client.query<Row>("SELECT * FROM sow_documents WHERE project_id=$1 AND organization_id=$2 ORDER BY created_at LIMIT 1 FOR UPDATE", [input.projectId, auth.organizationId]);
      let documentId = document.rows[0] ? String(document.rows[0].id) : randomUUID();
