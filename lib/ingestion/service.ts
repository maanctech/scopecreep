...
export async function persistManualMessages(client: PoolClient, input: {
  organizationId: string;
  projectId: string;
  ...
  if (!connection.rows[0])
    await client.query(
      ...
      [connectionId, input.organizationId],
    );
  ...
  const source = await client.query<{ id: string }>(
    ...
    [input.organizationId, connectionId, sourceExternalId],
  );
  ...
  if (!source.rows[0])
    await client.query(
      ...
      [sourceId, input.organizationId, connectionId, input.projectId, sourceExternalId],
    );
  ...
  await client.query(
    ...
    [
      ...
      connectionId,
      input.projectId,
      JSON.stringify({ format: input.preview.format, messageCount: input.preview.messages.length }),
      idempotencyKey,
    ],
  );
  let inserted = 0;
  ...
  for (const message of input.preview.messages) {
    ...
    const result = await client.query(
      ...
      [
        randomUUID(), input.organizationId, input.projectId, sourceId, linkedThread,
        message.externalId, message.sender, message.senderEmail, JSON.stringify(message.recipients),
        message.subject, message.text, message.timestamp, message.editedTimestamp, contentHash(message),
    ...
    if (result.rowCount) {
      ...
      if (linkedThread)
        await client.query(
          ...
          [message.timestamp, linkedThread, input.organizationId],
        );
    } else duplicates += 1;
  }
  ...
  await client.query(
    ...
    [JSON.stringify(result), jobId, input.organizationId],
  );
  await client.query(
    ...
    [
      randomUUID(), input.organizationId, input.actorUserId, jobId,
...
export async function importManualMessages(input: {
  ...
  return transaction((client) => {
    return persistManualMessages(client, {
      ...input,
