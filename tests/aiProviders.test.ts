...
describe("provider catalog", () => {
  ...
  it("reports only the credentials a provider actually needs", () => {
    ...
    expect(missingEnvironmentFor("openai", { OPENAI_API_KEY: "set" })).toEqual([]);
  });

  it("requires an API key from every provider it can now select", () => {
    for (const name of CATALOG_PROVIDER_NAMES) {
      expect(missingEnvironmentFor(name, {})).not.toEqual([]);
      expect(displayModelFor(name)).not.toBe("Automatic model selection");
    }
  });
});
